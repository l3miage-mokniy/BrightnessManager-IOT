...
accesstoken='ReEkT44yAedyqYcGsOvn'
...
async def run(address, debug=False):
	...
	async with BleakClient(address) as bleak_client:
		...
		def notification_handler(sender, data):
			timestamp, lux, power, presence = struct.unpack('<HiHh', data)
			msg = {'timestamp': timestamp,  'lux':lux, 'power':power , 'presence':presence}
			print(msg)
			r = requests.post(host, json=msg, verify='./chain.pem')

		#def notification_handler(sender, data):
			#timestamp, lux = struct.unpack('<HiHh', data)
			#msg = {'timestamp': timestamp, 'lux': lux}
			#print(msg)
			#r = requests.post(host, json=msg, verify='./chain.pem')
